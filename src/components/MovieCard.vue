...
const posterPath = computed(() => "https://image.tmdb.org/t/p/w500/" + props.movie.poster_path)

const movieGenres = computed(() =>
  props.genres.filter(genre =>
    props.movie.genre_ids.includes(genre.id)).map(g =>
      g.name).join(', '))
...
<template>
  ...
  <div class="mt-2">
    ...
    <div class="dark:text-gray-400 text-gray-500 text-sm">{{ movieGenres }}</div>
  </div>
</template>
